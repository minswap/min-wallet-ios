...
struct TokenLogo: View {
  ...
  var body: some View {
    ZStack {
      Group {
        if token.currencySymbol.isEmpty && token.tokenName.isEmpty {
          ...
          AsyncImage(
            ...
            Circle()
              .fill(Color.appSurfacePrimarySub)
              .overlay(
                ProgressView()
      ...
      if token.isVerified {
        Circle()
          .fill(.appBaseBackground)
          .frame(width: 16, height: 16)
          .overlay(
            AppIcon(name: IconName.verifiedBadge, size: 12, color: .appToneHighlight)
          )
          .overlay(
            Circle()
              .stroke(Color.appSurfacePrimarySub, lineWidth: 1)
          )
          .position(x: 26, y: 26)
      }
    }.frame(width: 28, height: 28)
