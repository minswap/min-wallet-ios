...
protocol GDomainTokenRefresher {
    ...
    ///
    /// ````
    /// func shouldRefreshTokenIfRequired(endpoint: GAPIRouter) -> RequiresRefreshResult {
...
protocol GDomainRetrier {
    func shouldRetry(endpoint: GAPIRouter, response: HTTPURLResponse?, data: Data?) throws -> RetryResult
}
...
class GDomainAuthRetrier {
    ...
    func shouldRetryDueToAuthenticationError(endpoint: GAPIRouter, response: HTTPURLResponse?, data: Data?) throws -> RetryResult {
    ...
    func _refresh() async throws -> Void {
        guard !isRefreshExcessive() else {
            throw GAPIRouterError.excessiveRefresh
        }
...
class GDomainRefreshRetrierNoOpDefault: GDomainRetrier, GDomainTokenRefresher {
    ...
    func shouldRetry(endpoint: GAPIRouter, response: HTTPURLResponse?, data: Data?) throws -> RetryResult {
        ...
        return .doNotRetryWithError(GAPIRouterError.serverUnauthenticated)
    }
    ...
    func requestRefreshToken() async throws {
        throw GAPIRouterError.localError(message: GAPIRouterError.GenericError)
    }

    func requestRefreshToken() -> AnyPublisher<Void, Error> {
        Fail<Void, Error>(error: GAPIRouterError.localError(message: "xxx"))
            .eraseToAnyPublisher()
    }
}
